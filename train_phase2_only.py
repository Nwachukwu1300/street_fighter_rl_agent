"""
Train ONLY Phase 2 - Medium Difficulty
Standalone training for Phase 2 to bypass checkpoint loading issues

Run with: caffeinate -d diambra run -s=4 python train_phase2_only.py
"""
...
def make_env(rank=0, difficulty=2, step_ratio=6):
...
def main():
    print("=" * 70)
    print("PHASE 2 TRAINING - Medium Difficulty")
    print("=" * 70)
    print("\nThis trains Phase 2 from scratch (not loading Phase 1)")
    print("The agent will still perform well due to:")
    print("  • Health-based rewards (continuous feedback)")
    print("  • Medium difficulty is a good learning baseline")
    print("  • 3M timesteps is enough to learn from scratch")
    print("=" * 70)
    ...
    num_envs = 4
    env = DummyVecEnv([make_env(i, difficulty=2, step_ratio=6) for i in range(num_envs)])
    env = VecMonitor(env)
    print(f"✓ Created {num_envs} environments (difficulty 2)")
    ...
    print()
    print("NEXT STEP:")
    print("  Train Phase 3 using this model:")
    print(f"  Edit train_phase3_only.py to load '{final_path}.zip'")
    print("=" * 70)
